...
public class BeerController {
    ...
    public ResponseEntity<Void> saveNewBeer(@RequestBody @Validated BeerDto beerDto){
        ...
        beerService.saveNewBeer(beerDto).subscribe(beerDtoResult -> atomicBeerId.set(beerDtoResult.getId()));

        return ResponseEntity
                .created(UriComponentsBuilder
                        .fromHttpUrl("http://api.springframework.guru/api/v1/beer/" + atomicBeerId)
                        .build().toUri())
                .build();
    }
    ...
    public ResponseEntity<Void> updateBeerById(@PathVariable("beerId") Integer beerId, @RequestBody @Validated BeerDto beerDto){
        ...
        beerService.updateBeer(beerId, beerDto).subscribe(savedNewDto -> {
            if (savedNewDto.getId() != null) {
